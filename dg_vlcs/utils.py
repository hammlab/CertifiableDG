...
def compute_WD(data_S, labels_S, data_T, labels_T, NUM_CLASSES, HEIGHT=28, WIDTH=28, NCH=1):
...
def load_VLCS(sources, targets):
    
    root = "path/DomainBed/domainbed/scripts/domainbed/data/VLCS"
...
def eval_accuracy_cdan(x_test, y_test, base_model, encoder, classifier, domain_classifier, loss_fn = 'ce'):
    ...
    for batch in range(nb_batches):
        ...
        combined_softmax_reshaped = tf.reshape(combined_softmax, [-1, 5, 1])
        domain_input = tf.matmul(combined_softmax_reshaped, combined_features_reshaped) 
        domain_input_reshaped = tf.reshape(domain_input, [-1, 128*5])
