...
from utils import load_VLCS, eval_accuracy, mini_batch_class_balanced, eval_accuracy_disc, compute_WD
from models import classificationNN, domain_predictor, representationNN
import argparse
...
# Load Dataset
print("Loading data")
src_data, src_labels, src_test_data, src_test_labels, target_data, target_labels = load_VLCS(SRCS, TRGS)
...
domain_classifier = domain_predictor(2, 128)
...
def train_step_da_1_pretrain(main_data, main_labels, domain_data, opposite_domain_labels):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        shared_domain = encoder(base_model(domain_data, training=False), training=True)
        domain_logits = domain_classifier(shared_domain, training=True)
...
for epoch in range(EPOCHS):
    ...
    for batch in range(nb_batches_train):
        ...
        if epoch >= PRETRAIN_STEPS:# and epoch % 5 == 0:
            #print("updating rep")
    ...
    if epoch % 1 == 0:
        ...
        print("Sources test:", srcs_test_accuracy)
        print("Target:", target_test_accuracy)
        print("Disc:", eval_accuracy(x_combined, y_dc_correct_combined, base_model, encoder, domain_classifier))
    ...
    if epoch%2 == 0 or epoch == PRETRAIN_STEPS-1:    
        ...
        WDs_sadv = compute_WD(encoded_src_train_images, src_train_labels, encoded_src_train_images_adv, src_train_labels, NUM_CLASSES=NUM_CLASSES)
        WDs_target = compute_WD(encoded_src_train_images, src_train_labels, encoded_trg_test_images, trg_test_labels, NUM_CLASSES=NUM_CLASSES)
        RHO = FACTOR * WDs_sadv
