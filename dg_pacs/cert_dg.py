...
from utils import load_PACS, eval_accuracy, mini_batch_class_balanced, eval_accuracy_disc, compute_WD, clip_eta
...
parser.add_argument('--NAME', type=str, default="DANN", help='DANN or WD')
...
CHECKPOINT_PATH = "./checkpoints/" + METHOD  + "_" + NAME + "_" + str(0)
...
# Load Dataset
print("Loading data")
_,_, src_test_data, src_test_labels, target_test_data, target_test_labels = load_PACS(SRCS, TRGS)
...
@tf.function
def train_step_adv_z(encoded_data_adv, encoded_data_orig, labels, GAMMA):#, need_opt_vec):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss = -(adv_loss - tf.cast(GAMMA, tf.float32) * distortion) #* need_opt_vec
...
#####################################################
RHOS = np.arange(0.0, 2.2, 0.2) * WD_Sadv
